...
import { testInfixExpression, InfixExpressionParserTest } from "./infix-expression-parser-test";
import { testIdentifier } from "./identifier-parser-test";

export function testIfExpression(stmt: Statement) {
  ...
  const consequence: ExpressionStatement = exp.consequence().statements()[0] as ExpressionStatement;
  testIdentifier(consequence.expression(), "x");
  expect(exp.alternative()).toBeNull();
}

export function testIfElseExpression(stmt: Statement) {
  expect(stmt).toBeInstanceOf(ExpressionStatement);
  const expressionStmt: ExpressionStatement = stmt as ExpressionStatement;
  expect(expressionStmt.expression()).toBeInstanceOf(IfExpression);
  const exp: IfExpression = expressionStmt.expression() as IfExpression;
  ...
  const alternative: ExpressionStatement = exp.alternative()?.statements()[0] as ExpressionStatement;
